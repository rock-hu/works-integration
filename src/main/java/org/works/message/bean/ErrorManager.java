...
import org.works.message.handler.DefaultHandler;
import org.works.message.handler.ExceptionHandler;
import org.works.message.handler.FatalHandler;
	...
	public void onException(ErrorMessage msg) throws MuleException {
		...
		try {
			...
			if (eh instanceof DefaultHandler) {
				logger.error(LocaleMessage
						.defaultFatalHandling(FatalHandler.class));
				handleFatal(e);

			} else if (eh instanceof FatalHandler) {
				logger.fatal(LocaleMessage.fatalHandling(e));
			} else {
				logger.error(LocaleMessage.defaultHandling(
						DefaultHandler.class, eh, e));
				handleDefault(msg, e);
			}
		}
	}
