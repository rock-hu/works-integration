...
public class BaseCustomComponent implements Callable, Initialisable, Disposable, MuleContextAware, Receiveable, Startable, Stoppable {
	...
	/**
	 * Keeps a list of any messages received on this service. Note that only
	 * references to the messages (objects) are stored, so any subsequent
	 * changes to the objects will change the history.
	 */
	...
	/**
	 * This method is used by some WebServices tests where you don' want to be
	 * introducing the {@link org.mule.api.MuleEventContext} as a complex type.
	 * 
	 * @param data
	...
	/**
	 * Always throws a {@link org.mule.tck.exceptions.FunctionalTestException}.
	 * This methodis only called if {@link #isThrowException()} is true.
	 * 
	...
	/**
	 * Will append the value of {@link #getAppendString()} to the contents of
	 * the message. This has a side affect that the inbound message will be
	 * converted to a string and the return payload will be a string. Note that
	 * the value of {@link #getAppendString()} can contain expressions.
	 * 
	 * @param contents
	 *            the string vlaue of the current message payload
	 * @param message
	 *            the current message
	...
	/**
	 * The service method that implements the test component logic. This method
	 * can be called publically through either
	 * {@link #onCall(org.mule.api.MuleEventContext)} or
	 * {@link #onReceive(Object)}
	 * 
	 * @param data
	 *            The message payload
	 * @param context
	 *            the current {@link org.mule.api.MuleEventContext}
	 * @return a new message payload according to the configuration of the
	 *         component
	 * @throws Exception
	 *             if there is a general failure or if
	...
	protected Object process(Object data, MuleEventContext context) throws Exception {
		...
		if (logger.isInfoEnabled()) {
			String msg = StringMessageUtils.getBoilerPlate(
					"Message Received in service: " + context.getFlowConstruct().getName() + ". Content is: " + StringMessageUtils.truncate(data.toString(), 100, true), '*', 80);
	...
	/**
	 * An event callback is called when a message is received by the service. An
	 * MuleEvent callback isn't strictly required but it is usfal for performing
	 * assertions on the current message being received. Note that the
	 * FunctionalTestComponent should be made a singleton when using MuleEvent
	 * callbacks
	 * <p/>
	 * Another option is to register a
	 * {@link org.mule.tck.functional.FunctionalTestNotificationListener} with
	 * Mule and this will deleiver a
	 * {@link org.mule.tck.functional.FunctionalTestNotification} for every
	 * message received by this service
	 * 
	...
	/**
	 * Often you will may want to return a fixed message payload to simulate and
	 * external system call. This can be done using the 'returnData' property.
	 * Note that you can return complex objects by using the
	 * <container-property> element in the Xml configuration.
	 * 
	 ...
	 * 
	 * @param returnData
	 *            the message payload to always return from this service
	 *            instance
	 */
	...
	/**
	 * Sometimes you will want the service to always throw an exception, if this
	 * is the case you can set the 'throwException' property to true.
	 * 
	 * @return throwException true if an exception should always be thrown from
	 *         this instance. If the {@link #returnData} property is set and is
	 *         of type java.lang.Exception, that exception will be thrown.
	 */
	 ...
	 *            true if an exception should always be thrown from this
	 *            instance. If the {@link #returnData} property is set and is of
	 *            type java.lang.Exception, that exception will be thrown.
	 */
	...
	/**
	 * If enableMessageHistory = true, returns the number of messages received
	 * by this service.
	 * 
	...
	/**
	 * If enableMessageHistory = true, returns a message received by the service
	 * in chronological order. For example, getReceivedMessage(1) returns the
	 * first message received by the service, getReceivedMessage(2) returns the
	 * second message received by the service, etc.
	 */
	...
	/**
	 * If enableMessageHistory = true, returns the last message received by the
